  ...
  chrome.storage.local.get(["activation"], ({ activation }) => {
    if (!activation) {
      document.body.insertAdjacentHTML(
        "beforeend",
        "<div class='no-extension d-none'></div>"
      );
      return;
    }

    const themes = document.querySelector(
      ".visually-hidden-fullscreen:has(.themes)"
    );
    const previewImages = document.querySelector(
      "div.input-group:has([for='image-preview-input'])"
    );
